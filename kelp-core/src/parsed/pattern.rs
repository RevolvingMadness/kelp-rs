    ...
    pub fn perform_semantic_analysis(
        ...
        Some(match (self.kind, inner_type) {
            ...
            (
                ...
                let pattern_id = path.get_type_id(ctx)?;
                let pattern_generic_spans = path.generic_spans;
                let pattern_generic_types = path.generic_types;

                let (pattern_id, pattern_type) = ctx.get_struct_id(
                    pattern_id,
                    &pattern_generic_spans,
            ...
            (
                ...
                let (pattern_id, pattern_declaration) =
                    ctx.get_tuple_struct(pattern_id, pattern_type, path.name_span)?;
                ...
                if expected_field_count != actual_field_count {
                    return ctx.add_error(SemanticAnalysisError::MismatchedTupleStructFieldCount {
                        name_span: path.name_span,
                        struct_id: pattern_id,
